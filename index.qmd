...
::: notes
This is where the idea of **modeling hubs** came in, to provide **centralised infrastructure for collaborative forecasting**.

-   First, Hubs **bring structure** and a **central location** for collecting model outputs.

-   They **establish a shared modeling protocol**.

-   By enforcing **standards** and **submission rules**, hubs ensure that **forecasts can be compared.**

-  and also **integrated into ensemble models**, often outperforming individual ones.

-   Finally, hubs make it easier for decision makers to **set the forecasting task** and stakeholders to **access timely, interpretable forecasts**.
...
::: notes
Before COVID, the US CDC had **already established a foundation for collaborative infectious disease forecasting through hubs** through its **FluSight initiative**, focused on influenza.
...
When the dust settled, this **motivated the development** of a more **configurable**, **generalized** and  **modular** system to support **new infectious disease modeling hubs.**
:::
...
::: notes
... and is **how the hubverse began**,

It's an **open-source software ecosystem** designed **to support the full lifecycle of infectious disease modeling hubs**.
...
They also **maintain `target-data`**: **observed data**, which is what predictions are scored against.

With that in place, different **modeling teams** submit model outputs of any type of model they want as long as it matches hub expectations and requirements.

To confirm this, each submission is **validated against the configuration**. If it passes, it's **merged**.
...
::: notes
All this is **enabled by the core of the hubverse** which is a **shared data standard**. 

The standard covers three things.

**One:** the **hub file structure** itself with standard directories. `hub-config` for config files, `model-output` for submissions, one directory per model, `model-metadata` for metadata files describing each model, and `target-data` for storing observed data.

**Two:** the **model output files themselves** follow a **consistent format** and **naming** convention.

**Three:** how a **modeling task** is defined, what's being predicted, what model output should look like etc.
...
::: notes
Digging into the `model-output` directory, we see:

**One:** one directory per model, named with the model's ID.

**Two:** one file per round inside that directory. 

**Three:** the file name is constructed from the round and model identifiers so is self-describing. In FluSight the round ID is a date, so the folder's contents read as that model's submission history.
...
::: notes
Now let's **talk about configuring a hub**. And start by taking a look at the **FluSight's config files**.
:::
...
- `tasks.json` for modeling tasks ie the scientific question needing answers. 
...
- and what every downstream tool uses to understand the hub.
...
``` {.bash code-line-numbers="|3-4|5|6-9|7|8|9"}
tasks.json
...
2. But if the rounds are driven by a variable in the data, 
    ...
    - that task ID's values become the round IDs but all other configuration is shared between rounds. 
    - That's what `round_id_from_variable: true` is flagging.

`submissions_due` sets the submission window.

Then model tasks. This is where a hub defines what it actually wants predicted. Each task is configured with three blocks.
...
``` {.json code-line-numbers="|18-21|7-10"}
"task_ids": {
  "reference_date": {
    "required": null,
...
- Optional means you may, but you don't have to. 
...
Also `target_end_date` is what we consider a derived task ID. Its values have a one to one relationship with other task IDs: here the reference date, which is also the round ID, plus the horizon in weeks gives you the target end date.
...
Some of that is descriptive. A human readable name, a fuller description. Useful for people, and for labelling an axis on a visualisation.

But it also carries quantitative properties. The units. Whether it's a step ahead forecast,  in what time unit. 
...
Notice there's no optional property here. Output type IDs are all required becaue a partial set breaks things downstream eg when evaluating or ensembling.
...
*(3)* Finally the **pmf** output type for a categorical rate-change outlook for a completely different target. Here output types are categories and values are propabilities that sum to one across individual predictions.
...
We ship a template of this schema and hubs are currently free to adapt it.
:::

# Beyond the data standard

::: notes
That's the standard, and how you configure it. Everything from here on we build on top of it.
:::
...
They all live in the `hubverse-actions` repo, and admins install them using the hubCI package.
:::
...
The mirror is synced as a parquet Arrow dataset which can be queried and filtered remotely before downloading data, in both R and Python. No cloning, or downloading a full hub's worth of data required.
...
::: notes
Finally because **dashboards matter** for communicating with a wider audience, we developed and support **a plug-and-play template** that admins can just copy and configure.

It's based on Quarto, so it's easy to customise.

Importantly it's fully static with no backend. GitHub workflows turn hub data into JSON, and a JavaScript frontend makes it interactive.
...
*(2, Then modelers)* who submit their model output can use `hubValidations` to check a submission before opening a pull request, and `hubData` to read hub data (including target data) back.

*(3, Analysts)* who want to perform analyses can use `hubData` to query and access data from the hub, `hubVis` to plot model outputs, `hubEnsembles` to build ensemmbles and `hubEvals` to evaluate model performance against the target data.

*(4, Stakeholders)*, who might be less technical and just want answers, can access insights via the dashboard (supported by the dashboard template). But if they need data, they can also download raw data directly from the cloud mirror.

*(5, Finally underneath it all, the hubverse developers and community)*. we maintain a bunch of additonal tools and templates, including increasingly AI skills, to support our activities.
...
As mentioned, every model output comes in as a pull request, and gets validated by CI which effectivelly deploys hubValidations functions.

On the left, we see submissions from different teams.

On the right, we see validation results which are posted to the PR. 

*(Passed tab)* When everything passes, you get a green tick, and the detail stays collapsed but still accessible.

*(Failed tab)* When it fails you get a red cross, check results expanded with failed checks highlighted. Here the only problem is that the submission arrived outside the submission window.
...
- further Content checks include: whether task ID and output type values are valid according to the config, whether every required task has been submitted and are predicted values within the bounds the config declared.

- Finally we also include output-type-specific ones. For example Quantile predicted values mustn't cross. `pmf` probabilities must to sum to one per task. The correct number of samples must be submitted, with a consistent compound task ID set.
...
Admins can configure extra checks uning the `validations.yml` config file. 
...
Beyond that, hubs can write their own custom check functions in R and register them in the same way. They run in the same pipeline and turn up in the same report.
...
We can then filter and collect data using standard `dplyr` verbs. In this example we filter for model output from a single model for a specific target end date before collecting data into our local R sesssion.
...
There are also hubs that might be collaborative but private
